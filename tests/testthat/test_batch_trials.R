...
test_that("testing edf.trials", {
  ...
  expect_is(tr,'list')
  expect_true(all(names(tr) %in% c('headers','fixations','saccades','blinks','messages')))
...
test_that("loading batch and combine.eyedata", {
  ...
  expect_equal(names(alldata[[1]]),c( "headers","fixations", "saccades", "blinks", "messages", "ID", "filename","header"))
